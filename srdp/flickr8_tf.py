...
class Caption_Generator():
    ...
    def __init__(self, n_words, dim_embed, dim_ctx, dim_hidden, n_lstm_steps, batch_size=200, ctx_shape=[196,512], bias_init_vector=None):
        ...
        with tf.device("/cpu:0"):
            ...
            #[n_words,dim_embed]是shape,-1是minval,1是maxval
        ...
        #bias weight这些值都是dim_hidden乘以4之后的维度
        ...
        with tf.variable_scope(tf.get_variable_scope()) as scope:
            for ind in range(self.n_lstm_steps):
                if ind == 0:
                    ...
                    tf.get_variable_scope().reuse_variables()#在整个作用域共享这些参数
                    with tf.device("/cpu:0"):
                        word_emb = tf.nn.embedding_lookup(self.Wemb, sentence[:,ind-1])#按索引查找

                x_t = tf.matmul(word_emb, self.lstm_W) + self.lstm_b # (batch_size, hidden*4) W*lstm_weight+b

                labels = tf.expand_dims(sentence[:,ind], 1)#在指定的index下添加1，专门用来制作one-hot变量
...
def preProBuildWordVocab(sentence_iterator, word_count_threshold=30): # borrowed this function from NeuralTalk
    ...
    for sent in sentence_iterator:
      ...
      for w in sent.lower().split(' '):
        word_counts[w] = word_counts.get(w, 0) + 1#生成一个字典键值对
    ...
    word_counts['.'] = nsents
    bias_init_vector = np.array([1.0*word_counts[ixtoword[i]] for i in ixtoword])#ixtoword[i]代表已经转变为id的索引
...
#############################
###### 잡다한 Parameters #####
annotation_path = './data/annotations_flickr8k.pickle'
feat_path = '/home/zzq/srdp_data/feats_flickr8k.npy'
...
def train(): # 전에 학습하던게 있으면 초기값 설정.
    ...
    wordtoix, ixtoword, bias_init_vector = preProBuildWordVocab(captions)#从caption 中进行提取
...
def test(test_feat='./guitar_player.npy', model_path='/home/zzq/srdp_data/model_8k/model.ckpt-99', maxlen=20):
    annotation_data = pd.read_pickle(annotation_path)
    captions = annotation_data['caption'].values
...
def test_tf(test_image_path='/home/zzq/srdp_image/test.jpg', model_path='/home/zzq/show_attend_and_tell.tensorflow/models/model-447', maxlen=30):
    ...
    ixtoword = np.load('/home/zzq/show_attend_and_tell.tensorflow/data/ixtoword_demo.npy').tolist()
